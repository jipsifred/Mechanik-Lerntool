...
export function TaskSidebar({ tasks, currentIndex, onSelect, isOpen }: TaskSidebarProps) {
  ...
  return (
    <div
      ...
      style={{
        width: visible ? '16%' : '0',
        minWidth: visible ? '160px' : '0',
        opacity: visible ? 1 : 0,
        marginLeft: visible ? '16px' : '0',
      }}
    >
      <div className="glass-panel-soft panel-radius p-6 flex flex-col h-full min-w-[160px]">
          ...
          {tasks.map((task, index) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 h-10 rounded-full text-body transition-colors flex items-center justify-between gap-2 ${
                  isActive
